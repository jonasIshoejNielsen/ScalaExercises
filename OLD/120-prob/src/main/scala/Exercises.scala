...
object Exercises {
  ...
  val BallsNo: Int = 8    //previous was 8
  ...
  def pick (n: Int): Element[Boolean] = Flip(1.0 / (n+1))   //black+the one red
  ...
  // '1'.
  //player.flatMap(r=> if (r) Constant(player) else move(next(player),n-1))
  def move (player: Player, n: Int): Element[Player] = for {
    r <- pick(n)
    p <- if (r) Constant(player) else move(next(player), n-1)
  } yield p
  ...
  println("probPeter: " + probPeter)
  
  // Which strategy is beter for Paula? What if BallsNo == 9?
  //
  // Write your answer here in a comment: for ballsNo==8: 0.493 if she starts and 0.496 if peter starts are the respective results.
  // For 9 balls we get 0.55 and 0.44, so here we see an advantage to starting.
  ...
  // each move(p,x) generates either a leaf: p or a node move(!p, x-1), with node move(p, 1) always giving only leaf p
      //thus there will be 10 leafs and two nodes 
  ...
  println("gameResult: " + Importance.probability(gameResult, Paula))

  // What is the probability that Paula wins with this uniform prior? Does it
  // agree with your intuition?
  // 0.504
  // We get about 50/50 chance that Paula started the game. This agrees with the intuition from propPaula and propPeter
  ...
  println("probPaulaStarted: " + probPaulaStarted)
  ...
  lazy val numBalls: List[Int] = List.tabulate(UpperBound)(identity _)
  lazy val blackBallsNo: Element[Int] = Uniform(numBalls:_*)
  ...
  lazy val posteriorOdd: Double = Importance.probability(blackBallsNo, (x:Int) => x % 2==1) // probability that the number of balls was odd
  println("posteriorOdd: " + posteriorOdd)
  // Is the posteriorOdd greater than 1/2? for UpperBound == 6: posteriorOdd=.5002


  // Reflect whether the above estimation would take you more time analytically
  // or with a probabilistic programming library?
  //fastests is: probabilistic
