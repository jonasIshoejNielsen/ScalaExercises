  ...
  object Laws {
    ...
    def homomorphism[B] (f: A => B) (mb: Monoid[B])
      (implicit arbA: Arbitrary[A]) = forAll{ (a1:A, a2:A) =>
        f(self.op(a1,a2)) shouldBe mb.op(f(a1), f(a2))   //distributive f(op(a,b)) = op(f(a), f(b))
        f(self.zero) shouldBe mb.zero                    //preserves identity f(zero)=f(zero)
      }
    ...
    def isomorphism[B: Arbitrary] (f: A => B, g: B => A) (mb: Monoid[B])
      (implicit arbA: Arbitrary[A]) = forAll{ (a1:A, a2:A) =>
        ...
        //Applied result is also horphism
        val b1 = f(a1)
        val b2 = f(a2)
        g(mb.op(b1,b2)) shouldBe self.op(g(b1), g(b2))   //distributive f(op(a,b)) = op(f(a), f(b))
        g(mb.zero) shouldBe self.zero                    //preserves identity f(zero)=f(zero)
...
object Monoid {
  ...
  def endoMonoid[A]: Monoid[A => A] = new Monoid[A => A] {
    ...
    def op (a1: (A => A), a2: (A => A)): (A => A)  = v=>a2(a1(v))
  ...
  // Exercise 5
    //NOTE: implicitly[Monoid[B]].zero
    //NOTE: implicitly[Monoid[B]].op(...)
...
class MonoidExercisesSpec
  ...
  "Exercise 7 (tests Exercise 6, written by student)" - {
    //isomorphism:  test converting list[char] to string and back is the same
...
trait Monad[F[_]] extends Functor[F] { self =>
  ...
  def sequence[A] (lfa: List[F[A]]): F[List[A]] = 
    lfa.foldRight[F[List[A]]] (unit(Nil)) (map2(_,_)(_::_) )
